...
import requestModel from "../../../models/agentRequest"
...
const userServices = {
  ...
  findUserWithPopulate: async (query) => {
    return await userModel.findOne(query).populate("wishList");
  },
  ...
  findUserforRequest: async (insertObj) => {
    return await userModel.findOne(insertObj);
  },

  findList : async (insertObj) => {
    return await requestModel.find(insertObj).populate("propertyId");
  },
}
